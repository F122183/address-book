...
const Dashboard = () => {
    ...
    const [sortBy, setSortBy] = useState<'firstName' | 'lastName' | 'createdAt' | null>(null);
    const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('desc');

    const fetchContacts = async () => {
    try {
        const { data } = await api.get(
            `/contacts?sortBy=${sortBy}&order=${sortOrder}`
        );
        setContacts(data);
    } catch (err) {
    ...
    const sortContacts = (contacts: Contact[]) => {
        if (!sortBy) return contacts; // няма сортиране
    ...
    const handleSort = (field: 'firstName' | 'lastName' | 'createdAt') => {
    if (sortBy === field) {
        setSortBy(null);
    } else {
        setSortBy(field);
                    ...
                    <Grid container spacing={2} alignItems="center">
                        ...
                        <Grid size={{ xs: 12 }}>
                            <Box display="flex" gap={1} flexWrap="wrap" mt={1}>
                                <Button
                                    size="small"
                                    variant={sortBy === 'firstName' ? 'contained' : 'outlined'}
                                    color={sortBy === 'firstName' ? 'primary' : 'inherit'}
                                ...
                                <Button
                                    size="small"
                                    variant={sortBy === 'lastName' ? 'contained' : 'outlined'}
                                    color={sortBy === 'lastName' ? 'primary' : 'inherit'}
